...
const createGroup = async (req, res) => {
  try {
    const { name, description, privacy, cover_url } = req.body;
...
const requestJoinGroup = async (req, res) => {
  try {
    ...
    const isBanned = await GroupMember.findOne({
      group: group_id,
      user: user_id,
      status: "banned",
    });
    if (isBanned) {
      return res
        .status(403)
        .json({ error: "You are banned from this group, can't join again." });
    }
  ...
  try {
    ...
    if (member.role === "admin") {
      return res
        ...
        .json({
          error:
            "Admins cannot leave the group. Please transfer admin rights to someone else first.",
        });
    }
  ...
  try {
    ...
    if (group.creator.toString() !== requester_id.toString()) {
      return res
        .status(403)
        .json({
          error: "Chỉ người tạo nhóm mới có quyền thay đổi quyền quản trị.",
        });
    }
...
const demoteOrTransferCreator = async (req, res) => {
  try {
    ...
    if (!["demote", "transfer_creator"].includes(action)) {
      return res
        .status(400)
        .json({ error: "Invalid action. Use 'demote' or 'transfer_creator'" });
    }
    ...
    if (action === "demote") {
      ...
      if (group.creator.toString() === current_user_id.toString()) {
        return res
          ...
          .json({
            error:
              "Creator cannot demote themselves. Transfer creator role first.",
          });
      }
    ...
    if (action === "transfer_creator") {
      if (group.creator.toString() !== current_user_id.toString()) {
        return res
          .status(403)
          .json({
            error: "Only the current creator can transfer creator role",
          });
      }
      ...
      if (!new_creator_id) {
        return res
          .status(400)
          .json({
            error: "new_creator_id is required for transfer_creator action",
          });
      }
      ...
      if (!newCreatorMember) {
        return res
          .status(400)
          .json({ error: "New creator must be an admin member of the group" });
      }
...
const updateGroup = async (req, res) => {
  try {
    const { group_id } = req.params;
    const { name, description, privacy, post_approval, cover_url } = req.body;
    const admin_id = req.user._id;
    const isAdmin = await isGroupAdmin(group_id, admin_id);
    if (!isAdmin) return res.status(403).json({ error: "Permission denied" });

    // Xử lý cover_url từ file upload (nếu có)
    let updateData = { name, description, privacy, post_approval, cover_url };

    const group = await Group.findByIdAndUpdate(group_id, updateData, {
      new: true,
      runValidators: true,
    });
    if (!group) return res.status(404).json({ error: "Group not found" });
...
const deleteGroup = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Group and all related data deleted successfully" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
