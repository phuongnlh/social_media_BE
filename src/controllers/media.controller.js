...
// Hàm chung để lấy media, nhận type là tham số
const _getUserMedia = async (req, res, mediaType) => {
  try {
    ...
    const postMedias = await PostMedia.find({
      ...
      .populate({
        path: "media_id",
        match: { media_type: mediaType }, // Lọc theo loại media (image/video)
        select: "url media_type createdAt",
      })
      .populate({
        path: "post_id",
    ...
    postMedias.forEach((postMedia) => {
      if (postMedia.media_id && postMedia.media_id.length > 0) {
        postMedia.media_id.forEach((media) => {
          if (media) {
            // Check if media exists after populate filter
            mediaList.push({
              _id: media._id,
              url: media.url,
    ...
    console.error(`Error in _getUserMedia (type: ${mediaType}):`, error);
    res.status(500).json({
      success: false,
...
const getUserImages = async (req, res) => {
  return _getUserMedia(req, res, "image");
};

const getUserVideos = async (req, res) => {
  return _getUserMedia(req, res, "video");
};
